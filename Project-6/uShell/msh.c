...
void eval(char *cmdline) 
{
    ...
    if(!builtin_cmd(argv)) { /* Checks if the request wasn't a built-in command */
        /* Creates mask for SIGCHLD signal */
        sigset_t mask;
        sigemptyset(&mask);
        sigaddset(&mask, SIGCHLD);

        sigprocmask(SIG_BLOCK, &mask, NULL); /* Blocks SIGCHLD */
        ...
        if(pid == 0) { /* Checks if is child process */
            sigprocmask(SIG_UNBLOCK, &mask, NULL); /* Unblocks SIGCHLD */
            setpgid(0,0); /* Sets unique process group ID */
...
int builtin_cmd(char **argv) 
{
    ...
    if(!strcmp(argv[0], "jobs")) {
        listjobs(jobs); /* prints list of jobs */
        return 1;
    }

    if((!strcmp(argv[0], "fg")) || (!strcmp(argv[0], "bg"))) {
        do_bgfg(argv); /* executes the fg or bg command */
        return 1;
    }
...
void do_bgfg(char **argv) 
{
    ...
    if(argv[1][0] == '%') { /* Checks if given the jid */
        /* Checks if not given a number */
    ...
    if(!strcmp(argv[0], "fg")) {
        kill(-job->pid, SIGCONT); /* Sends SIGCONT signal */
        job->state = FG; /* Sets job state to FG */
        waitfg(job->pid); /* Waits for foreground job to complete */
    }

    if(!strcmp(argv[0], "bg")) {
        kill(-job->pid, SIGCONT); /* Sends SIGCONT signal */
        job->state = BG; /* Sets job state to BG */
...
void waitfg(pid_t pid)
{
    /* sleeps while the foreground process is the given pid */
...
void sigchld_handler(int sig) 
{
    ...
    while((pid = waitpid(-1, &status, WNOHANG|WUNTRACED)) > 0) { /* While there are children running */
        if(WIFEXITED(status)) { /* Checks if the job exited normally */
            deletejob(jobs, pid); /* Reaps child */
        } else if(WIFSTOPPED(status)) { /* Checks if the job is stopped */
            printf("Job [%d] (%d) stopped by signal %d\n", pid2jid(pid), pid, WSTOPSIG(status));
            fflush(stdout);
            fflush(stdout);
            getjobpid(jobs, pid)->state = ST; /* Sets job state to ST */
        } else if(WIFSIGNALED(status)) { /* Checks if the job exited due to a signal */
...
void sigint_handler(int sig) 
{
    ...
    if(pid != 0) {
        kill(-pid, sig); /* Sends SIGCINT signal to forground job */
    }
...
void sigtstp_handler(int sig) 
{
    ...
    if(pid != 0) {
        kill(-pid, SIGTSTP); /* Sends SIGTSTP signal to forground job */

        struct job_t *job = getjobpid(jobs, pid);
        if(job != NULL) {
            job->state = ST; /* Sets job state to ST */
        }
    }
